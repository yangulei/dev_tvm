...
def benchmark(name, batch_size, warmup=20, repeat=5, steps=20, target="llvm", profiling=False):
    ...
    results["mxnet_time_mean"] = mxnet_time_mean
    results["mxnet_time_std"] = mxnet_time_std
    ...
    # for engine, dtype in [('byoc', 'fp32'), ('byoc', 'bf16')]:
    for engine, dtype in [('byoc', 'fp32')]:
    # for engine, dtype in [('tvm', 'fp32'), ('tvm', 'bf16'), ('byoc', 'fp32'), ('byoc', 'bf16')]:
    # for engine, dtype in [('tvm', 'fp32'), ('byoc', 'fp32')]:
        ...
        with tvm.transform.PassContext(opt_level=3):
            opt_mod, opt_params = relay.build_module.optimize(
                mod, target=target, params=params)
        with open('mod_{}_{}-{}_opt.swift'.format(name, engine, dtype), 'w') as fout:
            fout.write(opt_mod.astext(show_meta_data=False))
        ...
        results["{}-{}_time_mean".format(engine, dtype)] = infer_time_mean
        results["{}-{}_time_std".format(engine, dtype)] = infer_time_std
        ...
        max_mse = 0
        out_numbers = module_runtime.get_num_outputs()
        for i in range(out_numbers):
            output = module_runtime.get_output(i).numpy()
            if dtype == "bf16":
                output = np_bf162np_float(output)
            mse = np.square(np.subtract(output, mxnet_output)).mean()
            if mse > max_mse:
                max_mse = mse
        results["{}-{}_MSE".format(engine, dtype)] = max_mse
        print('{}: {}-{} MSE: {}'.format(name, engine, dtype, max_mse))
        print("{}: {}-{} result[0:12]: \n{}".format(name,
...
if __name__ == "__main__":
    ...
    for name in names:
        try:
            res = benchmark(name, args.batch_size, warmup=args.warmup, repeat=args.repeat,
            ...
            print(e)
            print('failed to bench "{}"'.format(name))
            continue
